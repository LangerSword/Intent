...
// Dynamic import: config.js is gitignored, so it may not exist.
// Fall back to safe defaults so the extension works without Supabase configured.
let CONFIG = {
...
try {
    ...
    // config.js not found — cloud sync will be disabled, local features still work
    console.warn('Intent: config.js not found. Cloud sync disabled. Copy config.example.js to config.js to enable.');
}
